...
int main()
{
    ...
    cout<<vec.empty()<<"\n";//to tell that array is empty or not in bool datatype 0 and 1.

    //how to insert an element at any position
    vec.insert(vec.begin()+1,40);//inserting at first position
    for(int i=0;i<vec.size();i++)
    cout<<vec[i]<<" ";
